...
- 按照编排任务的顺序，依次实现每个子任务。
  1. **定位代码段**：根据子任务的描述，定位到相关的代码段，读取上下文实现方式和规范。
  2. **查询相关文档**：根据“离线文档导航”章节的指引，精确查询相关文档。这些文档将作为开发的参考资料；MEMORY 中的经验也可以参考。
  3. **实现子任务**：编写代码实现子任务的功能。

- 每个子任务完成后，进行代码检查 `codelinter -f json --fix .`，并及时修复 `"severity":"error"` 级别的问题。
- 重复以上步骤，直到所有子任务完成。
- 若项目启用了 Git，询问用户之后再提交。
- 若安装了其它 coding 类技能，也一并启用它们。
